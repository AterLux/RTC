 ...
 * UARTBootloader.asm
 *
 * Это немного модифицированная версия универсального загрузчика https://github.com/AterLux/UARTBootloader
 *
 * Author: Dmitry Pogrebnyak (http://aterlux.ru/)
 *
 * Отличие от стандартного загрузчика:
 * 1) Производится чтение значения OSCCAL из EEPROM, и если оно задано (не равно 0xFF) то регистр OSCCAL обновляется этим значением
 * 2) При появлении флага TOV2 в TIMSK2 значение флага сбрасывается и производится увеличение на 1 32-битного числа, хранящегося в регистрах OCR2B:OCR2A:GPIOR2:GPIOR1
 *    Таким образом, если таймер реального времени запущен при запуске загрузчика, при помощи этих регистров он продолжает считать время.
 */ 
...
// Модификация RTC!
// Обновление предделителя 1:2
  ldi r16, (1 << CLKPCE);
  sts CLKPR, r16
  ldi r16, (1 << CLKPS0); // 1:2
  sts CLKPR, r16

// Загрузка OSCCAL
  ldi r16, high(MAGIC_EEPROM_ADDRESS)
  out EEARH, r17
  ldi r16, low(MAGIC_EEPROM_ADDRESS);
  out EEARL, r16
  sbi EECR, EERE
  in r16, EEDR
  cpi r16, 0xFF
  breq no_update_osccal
    sts OSCCAL, r16
  no_update_osccal:
...
// Обновление счётчика времени
  sbis TIFR2, TOV2
  rjmp skip_rtc_update
    sbi TIFR2, TOV2
    in r24, GPIOR1 // Для экономии памати используем 16-битную пару. Используем r25:r24, поскольку блинкер отключен, а кроме него счётчик никак не используется
    in r25, GPIOR2
    adiw r25:r24, 1
    out GPIOR1, r24
    out GPIOR2, r25
    brcc skip_rtc_update
...
clear_ports:
  clr r16
  .if (PC > (FLASHEND - 24)) // Если у нас слишком дофига портов, а памяти уже не осталось...
    ...
    .ifdef FORCE_METHOD
       out FORCE_DDR, r16
       out FORCE_PORT, r16
      .if (FORCE_METHOD >= 3) && (FORCE_DDR2 != FORCE_DDR)
         out FORCE_DDR2, r16
         out FORCE_PORT2, r16
      .endif
    .endif
